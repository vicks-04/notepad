...
export default function EditorPage() {
  ...
  const noteTasksBase = useTasksList({
    noteId,
    limit: 8,
    sortBy: "dueDate",
    sortOrder: "asc"
  ...
  useEffect(() => {
    ...
    async function fetchNote() {
      ...
      try {
        ...
        setSaveMessage(`Last saved ${new Date(response.note.updatedAt).toLocaleString()}`);
        setActionMessage("");
      } catch (err) {
        if (err.status === 401) {
          logout();
  ...
  const syncNote = (nextNote, nextActionMessage = "") => {
    ...
    setSaveMessage(`Last saved ${new Date(nextNote.updatedAt).toLocaleString()}`);
  ...
  const persistNote = async (mode = "manual") => {
    ...
    try {
      const response = await api.notes.update(token, noteId, {
        title,
        content,
        tags: parsedTags
      });
  ...
  useEffect(() => {
    if (!note || note.isDeleted || !isDirty || saving || autosaving) {
      return undefined;
    }
  ...
  const handleCreate = async () => {
    try {
      const nextNote = await notesState.createNote({
        title: "Untitled note",
        content: "",
        tags: []
      });
  ...
  const handleTrash = async (selectedNoteId) => {
  ...
  const handleRestore = async (versionId) => {
    const confirmed = window.confirm("Restore this older version? Your current note will be kept as a new snapshot.");
  ...
  const handleTogglePin = async (selectedNoteId = noteId) => {
    try {
      ...
      if (selectedNoteId === noteId) {
        syncNote(updatedNote, updatedNote.isPinned ? "Pinned" : "Unpinned");
      }
    } catch (err) {
  ...
  const handleArchiveToggle = async (selectedNoteId = noteId) => {
    try {
      ...
      if (selectedNoteId === noteId) {
        syncNote(updatedNote, updatedNote.isArchived ? "Archived" : "Restored from archive");
      }
    } catch (err) {
  ...
  const handleRestoreArchived = async (selectedNoteId) => {
    try {
      const updatedNote = await notesState.restoreArchivedNote(selectedNoteId);

      if (selectedNoteId === noteId) {
        syncNote(updatedNote, "Restored from archive");
      }
    } catch (err) {
      setError(err.message || "Unable to restore archived note.");
    }
  };

  const handleRestoreFromTrash = async (selectedNoteId = noteId) => {
    try {
      const updatedNote = await notesState.restoreFromTrash(selectedNoteId);

      if (selectedNoteId === noteId) {
        syncNote(updatedNote, "Restored from trash");
      }
    } catch (err) {
      setError(err.message || "Unable to restore note from trash.");
    }
  };

  const handlePermanentDelete = async (selectedNoteId = noteId) => {
    const confirmed = window.confirm("Delete this note forever?");

    if (!confirmed) return;

    try {
      await notesState.permanentlyDeleteNote(selectedNoteId);
  ...
  return (
    <AppShell
      notesProps={{
        ...
        onRestoreTrash: handleRestoreFromTrash,
    ...
    >
      <section className="editor-page">
        {loading ? <div className="panel-message">Loading note...</div> : null}
        ...
        {!loading && note ? (
          <>
            <NoteEditorForm
              ...
              onTrash={() => handleTrash(noteId)}
              onRestoreFromTrash={() => handleRestoreFromTrash(noteId)}
              onPermanentDelete={() => handlePermanentDelete(noteId)}
              onShare={handleShare}
              isSaving={saving}
            ...
            <TaskListPanel
              ...
              subtitle="Add action items that stay attached to this note."
